...
### 7. CROSS-PRODUCT COMPATIBILITY (Companion Apps)

The Wealth Hub is designed to coexist with specialized high-performance tools like the **ARUN/Titan Trading Bot**.

#### Read-Only Integration Bridge
To maintain absolute security and performance, FinFlow interacts with the Trading Bot via a **passive read-only bridge**:
- **Data Source**: FinFlow reads from the bot's `bot_state.json` or shared positions table.
- **Security**: FinFlow has **ZERO** write access to the bot's credentials or trading engine.
- **Fail-Safe**: If the Web UI is under high load, the Trading Bot remains unaffected as it runs in a separate process/container.

#### Ecosystem Vision
- **FinFlow**: For Family & Friends to track total wealth, PnL, and manual asset growth.
- **ARUN Bot**: For advanced users who want active algorithmic execution with the same brokers.
